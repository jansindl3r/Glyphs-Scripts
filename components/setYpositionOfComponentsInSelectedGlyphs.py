...
class Dialog:
    def __init__(self):
        ...
        self.window.label = vanilla.TextBox(
            (15, 15, -15, 20), "Y position", sizeStyle="small"
        )
        self.window.input_field = vanilla.EditText(
            (15, 40, -15, 20), sizeStyle="small", callback=self.on_input
        )
        self.window.process_all_layers = vanilla.CheckBox(
            (15, 70, -15, 20),
            "Process all master layers",
            value=True,
            sizeStyle="small",
        )
        self.window.button = vanilla.Button(
            (15, 100, -15, 20), "Apply", sizeStyle="small", callback=self.setYPosition
        )
            ...
            for glyph in font.glyphs:
                if glyph.selected:
                    process_glyph(
                        glyph,
                        value,
                        self.window.process_all_layers.get(),
                        selected_layers,
                    )
