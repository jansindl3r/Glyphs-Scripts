...
class Dialog:
    def __init__(self):
        ...
        self.window.select_glyph = vanilla.Button(
            (15, 15, -15, 20),
            "Select Glyph as Input",
            sizeStyle="small",
            callback=self.selected_glyph_as_input,
        )
        self.window.open()
    ...
    def selected_glyph_as_input(self, sender):
        ...
        if not getattr(self.window, "position_label", None):
            y = 35
            self.window.position_label = vanilla.TextBox(
                (-145, y, -80, 20), "Position", alignment="center"
            )
            self.window.settings_label = vanilla.TextBox(
                (-80, y, -15, 20), "Settings", alignment="center"
            )
        y = 55
        ...
        for c, component in enumerate(selected_layer.components):
            ...
            if duplicate_index is not None:
                ...
                setattr(
                    self.window,
                    f"component_{sanitized}_label",
                    vanilla.TextBox(
                        (15, y, -15, 20), component.name + f" ({duplicate_index + 1})"
                    ),
                )
            else:
                setattr(
                    self.window,
                    f"component_{sanitized}_label",
                    vanilla.TextBox((15, y, -15, 20), component.name),
                )
            setattr(
                self.window,
                f"component_{sanitized}_position",
                vanilla.CheckBox(
                    (-145 + button_center_offset, y, -80, 20), "", value=True
                ),
            )
            setattr(
                self.window,
                f"component_{sanitized}_settings",
                vanilla.CheckBox(
                    (-80 + button_center_offset, y, -15, 20), "", value=True
                ),
            )
            self.component_elements.append(f"component_{sanitized}")
            y += 20

        self.window.select_glyph._nsObject.setTitle_(
            f'Selected Glyph: "{selected_glyph.name}"'
        )
        ...
        if getattr(self.window, "paste_selected", None):
            ...
            self.window.paste_selected = vanilla.Button(
                (15, y, -15, 20),
                "Paste Selected",
                sizeStyle="small",
                callback=self.paste_selected,
            )
    ...
    def paste_selected(self, sender):
        ...
        for component in self.selected_layer.components:
            selected_layer_components_map.setdefault(component.name, []).append(
                component
            )
            ...
            for component in layer.components:
                ...
                if component.name in selected_layer_components_map:
                    if getattr(self.window, f"component_{sanitized}_position").get():
                        ...
                        component.position = selected_layer_components_map[
                            component.name
                        ][duplicate_index].position

                    if getattr(self.window, f"component_{sanitized}_settings").get():
                        for key, value in selected_layer_components_map[component.name][
                            duplicate_index
                        ].smartComponentValues.items():
