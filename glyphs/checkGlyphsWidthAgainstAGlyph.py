...
class Dialog:
	def __init__(self):
		...
		self.w.group.runButton = Button("auto", "Check Widths", callback=self.check_widths)
				...
				if re.match(pattern, glyph.name):
					for layer in glyph.layers:
						if layer.isMasterLayer or layer.isSpecialLayer:
							source_width = check_against[layer.layerId if layer.isMasterLayer else layer.associatedMasterId]
