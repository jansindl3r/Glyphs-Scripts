...
class Dialog:
	def __init__(self):
		...
		self.window.base_axis_index_label = vanilla.TextBox("auto", "Select Base Non-Italic Axis", sizeStyle="small")
		self.window.base_axis_index = vanilla.RadioGroup("auto", options, sizeStyle="small", callback=self.on_input)
		self.window.check = vanilla.Button("auto", "Check", sizeStyle="small", callback=self.check)
	...
	def check(self, sender):	
		...
		for master in self.font.masters:
			groupped_masters.setdefault(master.axes[base_axis_index], []).append(master.id)
